...
description: Use when releasing or publishing @bjorntech/alchemy-azure; covers version bumps, changelog promotion, compatibility docs, gates, GitHub trusted publishing, GitHub Releases, and npm verification.
license: MIT
...
- Keep the Alchemy beta suffix aligned with the pinned dependency line, for example `0.1.0-beta.57` with `alchemy@2.0.0-beta.57`.
...
- Confirm the target version is not already published: `npm view "@bjorntech/alchemy-azure@<version>" version`.
- Run all gates in order: `bun run check`, `bun test`, `bun run coverage:check`.
- Run `git diff --check`.
...
- Ensure npm dist-tags are updated after publish: every release should point `latest` at the released version, and beta/prerelease versions should also point `next` at the released version.
...
- Verify npm after workflow success: `npm view "@bjorntech/alchemy-azure@<version>" version dist-tags`.
- Confirm npm dist-tags point to the released version: `latest` for every release, plus `next` for beta/prerelease versions.
